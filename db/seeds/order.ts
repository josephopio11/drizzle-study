  ...
  await Promise.all(
    orders.map(async (order) => {
      ...
      const foundAddress = await db.query.address.findFirst({
        where: eq(schema.address.streetAddress1, order.delivery_street_address),
      });
      if (!foundAddress) {
        throw new Error(
      ...
      await Promise.all(
        menuItems.map(async ({ dbItem, item }) => {
      ...
      const startTime = new Date(insertedOrder.createdAt);
      const endTime = new Date(insertedOrder.actualDeliveryTime || startTime);
      const total = endTime.getTime() - startTime.getTime();
      const increment = total / statuses.length;
      let currentTime = startTime;
      for (const statusCatalog of statuses) {
